...
public class SwerveDriveSimulation3D extends AbstractDriveTrainSimulation3D {
    private static final double YAW_DAMPING_COEFFICIENT = 1.0;
    private static final double PITCH_DAMPING_COEFFICIENT = 120.0;
    private static final double ROLL_DAMPING_COEFFICIENT = 120.0;
    private static final double PITCH_LEVELING_COEFFICIENT = 180.0;
    private static final double ROLL_LEVELING_COEFFICIENT = 180.0;
     ...
     *     {@link Pose2d}
     */
    public SwerveDriveSimulation3D(DriveTrainSimulationConfig config, Pose3d initialPoseOnField) {
    ...
    public SwerveDriveSimulation3D(DriveTrainSimulationConfig config, Pose2d initialPoseOnField) {
        this(config, new Pose3d(initialPoseOnField));
    }
    ...
    public void simulationSubTick() {
        ...
        gyroSimulation.updateSimulationSubTick(getAngularVelocity());
        applyYawDamping(YAW_DAMPING_COEFFICIENT);
        applyPitchDamping(PITCH_DAMPING_COEFFICIENT);
        applyRollDamping(ROLL_DAMPING_COEFFICIENT);
        applyPitchLeveling(PITCH_LEVELING_COEFFICIENT);
        applyRollLeveling(ROLL_LEVELING_COEFFICIENT);
    }
    ...
    private void simulateChassisFrictionForce() {
        ...
        final double FRICTION_FORCE_GAIN = 14.0,
                totalGrippingForce =
        ...
        final double
                ...
                grippingTorqueMagnitude =
                        moduleSimulations[0].config.getGrippingForceNewtons(gravityForceOnEachModule)
                                ...
                                * moduleSimulations.length,
                FRICTION_TORQUE_GAIN = 6.5;

        if (actualRotationalMotionPercent < 0.01 && desiredRotationalMotionPercent < 0.02) setAngularVelocity(0);
        else
            applyTorqueZ(Math.copySign(
                    Math.min(
    ...
    private void simulateModulePropellingForces() {
        for (int i = 0; i < moduleSimulations.length; i++) {
            ...
            final Vector2 moduleForce = moduleSimulations[i].updateSimulationSubTickGetModuleForce(
                    getLinearVelocity(moduleWorldPosition),
